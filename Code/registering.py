...
class register_handler:
    ...
    def pad_pic(self, picture):
        # fill with zeros till reach the wanted size
        return picture

    def crop_pic(self, picture):
        # resize the image with its original 
    ...
    def resize_picture(self, picture):
        if picture.shape[0] < self.pic_shape[0] or picture.shape[1] < self.pic_shape[1]:
            picture = self.pad_pic(picture)
        elif picture.shape[0] > self.pic_shape[0] or picture[1] > self.pic_shape[1]:
            picture = self.crop_pic(picture)
        return picture

    def read_next_picture(self):
        name = self.name_pic[self.index][0]
        picture = Image.open(self.db_path +
                             name + '/' +
...
def register() -> None:
    ...
    for i in tqdm(range(number_of_pictures), desc='Photos Done', ncols=75, unit='Photos'):
