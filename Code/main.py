...
def recog():
    ...
    while True:
        ...
        ret = face_rec.recog(objID, selected_face, all_IDs, align=False, detector_backend='skip')
        if ret is not None:
...
def track():
    ...
    while True:
        ...
        if not queue_yolo_track.empty():
            ...
            for rect in rects:
                rec.append([int(rect[0]*s[0]), int(rect[1]*s[1]),
                            int(rect[2]*s[0]), int(rect[3]*s[1])])
            rects = rec

        objects = tr.objects(rects)
        tr.ct.rewire_center_rects(rects)
        
        if is_moving:
            for ID, rect in tr.ct.rects.items():
                if ID[0:3] == 'ID ' or ID[0:3] == 'unk':
                    if ID in ID_sent.keys():
                        ID_sent[ID] -= 1
                        if ID_sent[ID] != 0:
                            continue
                        else:
                            ID_sent[ID] = ID_resend
                            queue_track_recog.put([ID, 
                                frame[rect[1]:rect[3], rect[0]:rect[2]]])
                    else:
                        ID_sent[ID] = ID_resend
                        queue_track_recog.put([ID, 
                                frame[rect[1]:rect[3], rect[0]:rect[2]]])
...
def display():
    ...
    while True:
        ...
        if key_press == ord('q'):
            ...
            print('Saving name')
            with open('.testing', mode='r') as file:
...
if __name__ == '__main__':
    ...
    p_recogs = [Process(target=recog) for _ in range(4)]
